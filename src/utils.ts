...
export function times<T = any>(n: number, fn: (index: number) => T): T[] {
  const results = [];
  for (let i = 0; i < n; i++) {
    results.push(fn(i));
  }
...
export function chunks<T = any>(list: T[], size: number): T[][] {
...
export function unique<T>(list: T[], fn: (element: T) => any): T[] {
  const exists: {[k:string]: boolean} = {}, result = [];
  for (let i = 0; i < list.length; i++) {
